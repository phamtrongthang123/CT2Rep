...
def _scanpath_to_string(scanpath, height, width, depth, n):
    ...
    string = ""
    # n = 5 so that n + n^2 = 30, end of ascii is 127, start from 97.
    # this metric is very limited.
    # I modify it this way to make sure it is not overflowed.
    for i in range(np.shape(scanpath)[0]):
        fixation = scanpath[i].astype(np.int32)
        correspondent_square = (
            (fixation[0] // width_step)
            + (fixation[1] // height_step) * n // 2
            + (fixation[2] // depth_step) * n // 2
        )
...
def create_k_vectors(arr, k):
    # Create a view of the array with shape (n, k)
    return np.lib.stride_tricks.sliding_window_view(arr, (k, arr.shape[1]))[:, 0, :, :]
...
def time_delay_embedding_distance(
    ...
    # def save_inputs_as_pkl(
    #     human_scanpath, simulated_scanpath, k, distance_mode, filename="inputs.pkl"
    # ):
    #     inputs = {
    #         "human_scanpath": human_scanpath,
    #         "simulated_scanpath": simulated_scanpath,
    #         "k": k,
    ...
    # save_inputs_as_pkl(human_scanpath, simulated_scanpath, k, distance_mode)
    ...
    if len(human_scanpath) < k or len(simulated_scanpath) < k:
        ...
        return False
    # Create k-vectors for both scanpaths
    human_scanpath_vectors = create_k_vectors(human_scanpath, k)
    # human_scanpath_vectors = []
    # for i in np.arange(0, len(human_scanpath) - k + 1):
    #     human_scanpath_vectors.append(human_scanpath[i : i + k])
    simulated_scanpath_vectors = create_k_vectors(human_scanpath, k)
    ...
    # Expand dimensions to create pairwise combinations
    simulated_expanded = simulated_scanpath_vectors[
        :, np.newaxis, :, :
    ]  # Shape: (999, 1, 2, 3)
    human_expanded = human_scanpath_vectors[
        np.newaxis, :, :, :
    ]  # Shape: (1, 999, 2, 3)

    # Compute Euclidean distances for each corresponding point
    pointwise_distances_0 = np.subtract(
        simulated_expanded[..., :2], human_expanded[..., :2]
    )
    # faster than np.power
    pointwise_distances_1 = pointwise_distances_0 * pointwise_distances_0
    pointwise_distances = np.sqrt(
        np.sum(pointwise_distances_1, axis=3)
    )  # We don't count the duration, only xy. Shape: (999, 999, 2)
    # Sum pointwise distances to get the total distance per scanpath
    total_distances = np.sum(pointwise_distances, axis=2)  # Shape: (999, 999)
    # Find the minimum distance for each simulated scanpath
    min_distances = np.min(total_distances, axis=1)
    distances = min_distances / k
    # distances = []
    ...
    # print(np.all(np.isclose(np.array(distances), distances2)))
    # return
