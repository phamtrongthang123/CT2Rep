...
class attention_module(nn.Module):
    ...
    def forward(self, features, attention_map, subject_embedding):
        ...
        attention_weights = F.softmax(
            self.attention(
                torch.tanh(
                    proj_aggr_feature + proj_subject_embedding.unsqueeze(1).unsqueeze(1)
                )
            ),
            2,
        )
...
class CrossAttentionPredictor(nn.Module):
    ...
    def forward(
        ...
        tgt = self.norm(tgt) # tgt.shape 7,1,768
        att = self.multihead_attn(
            query=self.with_pos_embed(tgt, querypos_embed),
...
class gazeformer(nn.Module):
    def __init__(
        ...
        self.attention_map_predictors = nn.ModuleList(
            [
                CrossAttentionPredictor(self.args.nhead, dropout, self.args.hidden_dim)
                for _ in range(action_map_num)
            ]
        )

        # attention modules for the output
        self.attention_module = attention_module(
            self.hidden_dim, subject_feature_dim, self.hidden_dim // 2
        )
    ...
    def init_weights(self):
        for modules in [
            self.subject_embed.modules(),
            self.querypos_embed.modules(),
            self.token_predictor.modules(),
            self.generator_t_mu.modules(),
            self.generator_t_logvar.modules(),
        ]:
            for m in modules:
                if isinstance(m, nn.Conv2d):
    ...
    def training_process(self, src, subjects, task):
        tgt_input = src.new_zeros(
            (self.max_len, src.size(0), self.hidden_dim)
        )  # Notice that this where we convert target input to zeros. src.shape 1,512,768. 512 = 8*8*8
        # tgt_input.shape 7,1,768
        # tgt_input[0, :, :] = self.firstfix_linear(self.queryfix_embed[tgt[:, 0], tgt[:,1], :])
        memory_task, outs = self.transformer(
            src=src,
            tgt=tgt_input,
            tgt_mask=None,
            tgt_key_padding_mask=None,
            subjects=self.subject_embed(subjects),
            task=task,
            querypos_embed=self.querypos_embed.weight.unsqueeze(1),
            patchpos_embed=self.patchpos_embed,
        ) # memory_task 512, 1, 768

        outs = self.dropout(outs)  # outs.shape 7,1,768
        # get Gaussian parameters for (t)
        t_log_normal_mu, t_log_normal_sigma2 = (
            self.generator_t_mu(outs),
            torch.exp(self.generator_t_logvar(outs)),
        )
        action_maps = []
        for attention_map_predictor in self.attention_map_predictors:
            action_map = attention_map_predictor(
                outs,
                memory_task,
                tgt_mask=None,
                memory_mask=None,
                ...
                querypos_embed=self.querypos_embed.weight.unsqueeze(1), # self.querypos_embed.weight.unsqueeze(1).shape 7,1,768
                patchpos_embed=self.patchpos_embed,
            )
            action_maps.append(action_map)
        action_maps = torch.stack(action_maps, dim=-2) # action_maps.shape 1,7,4,512
        # get the attention weight
        attention_weights = self.attention_module(
            memory_task, action_maps, self.subject_embed(subjects)
        ) # attention_weights.shape 1,7,4
        ...
        # [N, T, H, W]
        predicts["action_map"] = aggr_action_map.view(
            -1, self.max_len, self.spatial_dim[0], self.spatial_dim[1], 8
        )
    ...
    def inference(self, src, subjects, task):
        tgt_input = src.new_zeros(
            (self.max_len, src.size(0), self.hidden_dim)
        )  # Notice that this where we convert target input to zeros
